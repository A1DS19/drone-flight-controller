...
| Warning | `pcb/sym-lib-table` now registers `custom-symbols` (microSD + the corrected `USB_B_Micro`, added 2026-07-20 after "Update Symbols from Library" twice restored the global lib's stale SH shield pin — J3/J16 now resolve project-locally, making that regression impossible). The DigiKey/PCM libraries still resolve only through the embedded cache and the user's global table. | `pcb/sym-lib-table` |
...
| USB-serial | `U4` CH340G (SOIC-16), `J16` micro-B (MicroXNJ), `Y4` 12 MHz |
